function EOF3(datafile,doplot,threshold,algo)
...
if algo == 0
  fprintf('Computing EOFs... (V0 Fortran)\n');
  v0=rand(nx*ny,12);
  maxit = 5000;
  epsi=1e-10;
  tic;
...
if algo == 1
  fprintf('Computing EOFs... (V1 Fortran)\n');
  v0=rand(nx*ny,12);
  maxit = 5000;
  epsi=1e-12;
  tic;
...
if algo == 2
  fprintf('Computing EOFs... (V2 Fortran)\n');
  v0=rand(nx*ny,12);
  maxit = 5000;
  epsi=1e-12;
  tic;
...
% knowndata = floor(ns/4):floor(3*ns/4);
knowndata = 1:(ns-1);
...
% Si les deux sont differents, la decomposition QR ne pourra pas se faire
nd
rankofVi = rank(Vi)
