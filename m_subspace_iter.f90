...
! On initialise V avec m vecteurs linéairement indép. de la base canonique de R^m
subroutine init(v,n,m)
	implicit none
	integer, intent(in)								:: n, m
	double precision, dimension(n,m), intent(inout)	:: v
	integer											:: i, j
...
lwork, res, it, ierr)
	...
	! On initialise V avec m vecteurs linéairement indép. de la base canonique de R^m, donc V est déjà orthonormée
	call init(v,n,m)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine subspace_iter_v1(a, v, w, n, m, percentage, maxit, eps, work,    &
	...
	double precision, dimension(m,m)						:: w_mat
	double precision, dimension(m,m)						:: h
	...
	do while ((percentReached <= percentage).and.(it<=maxit).and.(n_ev<m))
		...
		! On met les valeurs propres sur la diagonale de w_mat
		do i=1,m
			w_mat(i,i) = w(i)
		end do
		...
		aux = norme2mat(matmul(a,v(:,m-n_ev))-w_mat(m-n_ev,m-n_ev)*v(:,m-n_ev),n)
		
		! Sauvegarde des vecteurs propres
		do while (aux < trace*eps) 
			it_ev(n_ev+1) = it                	
			res_ev(n_ev+1) = aux
			percentReached = percentReached + w_mat(m-n_ev,m-n_ev)/trace 			
			n_ev = n_ev + 1 
			if (n_ev.gt.m) then
			   aux = trace*eps+1 
			else
				aux = norm2v(matmul(a,v(:,m-n_ev))-w_mat(m-n_ev,m-n_ev)*v(:,m-n_ev),n)
			end if	
		end do 
